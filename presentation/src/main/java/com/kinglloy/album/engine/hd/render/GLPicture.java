...
import com.kinglloy.album.engine.style.render.*;
...
class GLPicture {
  ...
  private static final int VERTEX_STRIDE_BYTES = COORDS_PER_VERTEX * com.kinglloy.album.engine.style.render.GLUtil.BYTES_PER_FLOAT;
  ...
  private static final int TEXTURE_VERTEX_STRIDE_BYTES = COORDS_PER_TEXTURE_VERTEX
      * com.kinglloy.album.engine.style.render.GLUtil.BYTES_PER_FLOAT;
  ...
  public static void initGl() {
    ...
    int vertexShaderHandle = com.kinglloy.album.engine.style.render.GLUtil.loadShader(GLES20.GL_VERTEX_SHADER, VERTEX_SHADER_CODE);
    int fragShaderHandle = com.kinglloy.album.engine.style.render.GLUtil.loadShader(GLES20.GL_FRAGMENT_SHADER, FRAGMENT_SHADER_CODE);

    sProgramHandle = com.kinglloy.album.engine.style.render.GLUtil.createAndLinkProgram(vertexShaderHandle, fragShaderHandle, null);
  ...
  public GLPicture(BitmapRegionLoader bitmapRegionLoader, int maxHeight) {
    ...
    mHasContent = true;
    mVertexBuffer = com.kinglloy.album.engine.style.render.GLUtil.newFloatBuffer(mVertices.length);
    mTextureCoordsBuffer = com.kinglloy.album.engine.style.render.GLUtil.asFloatBuffer(SQUARE_TEXTURE_VERTICES);
    ...
    for (int y = 0; y < mRows; y++) {
      for (int x = 0; x < mCols; x++) {
        ...
        if (useBitmap != null) {
          mTextureHandles[y * mCols + x] = com.kinglloy.album.engine.style.render.GLUtil.loadTexture(useBitmap);
  ...
  public GLPicture(Bitmap bitmap) {
    ...
    mTextureHandles = new int[mCols * mRows];
    if (mCols == 1 && mRows == 1) {
      mTextureHandles[0] = com.kinglloy.album.engine.style.render.GLUtil.loadTexture(bitmap);
    } else {
      Rect rect = new Rect();
      for (int y = 0; y < mRows; y++) {
        for (int x = 0; x < mCols; x++) {
          ...
          mTextureHandles[y * mCols + x] = com.kinglloy.album.engine.style.render.GLUtil.loadTexture(subBitmap);
          subBitmap.recycle();
        }
      }
    }
  ...
  public void draw(float[] mvpMatrix, float alpha) {
    ...
    com.kinglloy.album.engine.style.render.GLUtil.checkGlError("glUniformMatrix4fv");
    ...
    for (int y = 0; y < mRows; y++) {
      for (int x = 0; x < mCols; x++) {
        ...
        com.kinglloy.album.engine.style.render.GLUtil.checkGlError("glBindTexture");
  ...
  public void destroy() {
    if (mTextureHandles != null) {
      ...
      com.kinglloy.album.engine.style.render.GLUtil.checkGlError("Destroy picture");

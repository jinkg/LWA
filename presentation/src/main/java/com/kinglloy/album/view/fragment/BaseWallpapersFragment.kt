...
abstract class BaseWallpapersFragment : Fragment(), WallpaperListView {
  ...
  override fun unSelectWallpaper(wallpaper: WallpaperItem) {
    var oldSelectedIndex = -1
    for ((index, value) in wallpapers.withIndex()) {
      if (value.isSelected) {
        value.isSelected = false
  ...
  private fun showPromote(): Int {
    val ultimate = PackageUtil.isPro(activity!!)
    if (ultimate) {
      return PROMOTE_NONE
    }
    if (adRandom == null) {
  ...
  private fun toRate() {
    val appPackageName = getString(R.string.ad_version_package_name)
    try {
      startActivity(
        Intent(
          Intent.ACTION_VIEW,
